...
fn translate_keycode(code: u16) -> u8 {
    match code {
        17 | 103 => b'w',   // W / Up arrow
        30 | 105 => b'a',   // A / Left arrow
        31 | 108 => b's',   // S / Down arrow
        32 | 106 => b'd',   // D / Right arrow
        28 => b'\r',         // Enter
        57 => b' ',          // Space
        63 => b'\x05',       // F5 -> save
        67 => b'\x09',       // F9 -> load
    ...
    impl Process for SyscallContext {
        ...
        fn spawn(&self, _caller: Caller, path: usize, count: usize) -> isize {
            ...
            let data = current
                ...
                .map(|fd| read_all(fd));
            let child = data
                .and_then(|d| ElfFile::new(&d).ok().and_then(|elf| ProcStruct::from_elf(elf)));
            match child {
                Some(child_proc) => {
                    let pid = child_proc.pid;
