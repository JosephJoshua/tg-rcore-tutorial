...
/// 物理内存容量 = 48 MiB
// Must not cover 0x8500_0000..0x8550_0000 (DMA pool) — the heap uses map() which
// allocates new pages, NOT identity-mapping. DMA needs identity-mapped physical addresses.
// Kernel at 0x80200000 + 72 MiB = 0x84A00000, safely below DMA pool at 0x85000000.
const MEMORY: usize = 72 << 20;
/// 异界传送门所在虚页
...
pub const MMIO: &[(usize, usize)] = &[
    (0x1000_0000, 0x00_9000),   // VirtIO MMIO range
    (0x8500_0000, 0x0050_0000), // DMA pool (must be identity-mapped for VirtIO DMA)
];
...
extern "C" fn rust_main() -> ! {
    ...
    // Initialize VirtIO GPU (needed before user apps can draw).
    // Keyboard is initialized later, after the block device Lazy init.
    #[cfg(target_arch = "riscv64")]
    {
        ...
        unsafe {
            ...
            // Store keyboard for later — don't init until after block device
...
mod impls {
            ...
            current.address_space
                ...
                .map_or_else(
                    ...
                    |fd| {
                        ...
                        log::info!("loading ELF: {} bytes", data.len());
                        current.exec(ElfFile::new(&data).unwrap());
                        0
                    },
                )
        }
    ...
    impl tg_syscall::Thread for SyscallContext {
        ...
        fn thread_create(&self, _caller: Caller, entry: usize, arg: usize) -> isize {
            ...
            let current_proc = unsafe { (*processor).get_current_proc().unwrap() };
            // 从最高用户栈位置向下搜索空闲的页表区域
            let mut vpn = VPN::<Sv39>::new((1 << 26) - 16);
            ...
            addrspace.map_extern(vpn..vpn + 16, PPN::new(stack as usize >> Sv39::PAGE_BITS), build_flags("U_WRV"));
