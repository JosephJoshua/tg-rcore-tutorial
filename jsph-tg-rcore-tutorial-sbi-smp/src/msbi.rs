...
///
/// SMP: 每个 hart 有独立的 CLINT mtimecmp 寄存器，地址为 0x2004000 + 8 * hartid。
fn handle_timer(time: u64) -> SbiRet {
    let hartid: usize;
    // SAFETY: 读取 mhartid CSR 是有效的 M-mode 操作。
    unsafe {
        core::arch::asm!("csrr {}, mhartid", out(reg) hartid);
    }
    let mtimecmp_addr = 0x200_4000 + 8 * hartid;
    // SAFETY: 向 QEMU virt 机器的 CLINT 寄存器写入。
    unsafe {
        (mtimecmp_addr as *mut u64).write_volatile(time);
    }
    // 清除挂起的 S-mode 定时器中断（STIP）。
