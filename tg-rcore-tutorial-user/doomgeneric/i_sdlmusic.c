...
static char *GetFullPath(char *base_filename, char *path)
{
    ...
    {
        p[1] = '\0';
        result = M_StringJoin2(basedir, path);
    }
    else
    {
        result = strdup(path);
...
static void LoadSubstituteConfigs(void)
{
    ...
    {
        musicdir = M_StringJoin3(configdir, "music", DIR_SEPARATOR_S);
    }
    ...
    {
        path = M_StringJoin2(musicdir, subst_config_filenames[i]);
...
void I_InitTimidityConfig(void)
{
    ...
    if (success)
    {
        env_string = M_StringJoin2("TIMIDITY_CFG=", temp_timidity_cfg);
        putenv(env_string);
    }
    else
    {
