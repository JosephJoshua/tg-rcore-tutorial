...
4. **初始渲染性能**：400 个格子 + 20 条背景条带的 fb_write 各触发一次 GPU flush，在 debug 模式下较慢。使用 40 行高的条带（而非 20 行）减少 fb_write 调用次数，首次渲染是一次性成本。
...
5. **背景填充在用户空间**：背景色填充通过用户侧 fb_write 条带完成，内核不预填充任何颜色（保持游戏无关性）。首次渲染约 20 条带 + 400 格子，是一次性成本。
